...
`prototype/index.html` 是高保真设计系统原型，作为 UI 实现的单一可信源参考。为避免原型与扩展代码脱节，约定如下：

- **版本号同步**：原型导航与页脚显示的版本号必须与 `manifest.json` 的 `version` 字段保持一致（当前 `1.6.13`）。每次发布 bump 版本时，原型版本号随文档一并更新。
- **设计令牌单一来源**：设计变量（颜色 / 间距 `--space-*` / 字体栈 / 圆角 / 阴影）以 `ui/tokens.js` 为权威，原型 `:root` 令牌与之对齐；改动令牌需两处同改，并补注释说明维护约定。
- **卡片结构对齐**：原型商品卡的信息层级、价格区（现价 / 起拍 / 划线原价 / 出价人数 / 倒计时）与 `ui/products.js` + `ui/price-render.js` 保持一致；原型改动后需回写本约定说明。
- **交互行为对齐**：搜索防抖、历史下拉（工具栏 focusout 收起）、分页「加载更多」（首屏 60 + 追加）等交互以 `ui/toolbar/*` + `ui/results.js` 为权威，原型须如实反映，不得用静态截断提示替代。
- **数据真实性**：原型演示数据应体现真实数量级（>60 条以演示分页）；英雄区指标仅展示诚实可验证项（商品总数 / 双语 / 防抖延迟 / 轻量注入式），不虚构延迟或包体数值。
